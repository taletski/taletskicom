...
//
// The dynamic server will still be the main project. For now, it will live on a temporary domain just as a POC.
//
...
fn main() {
    ...
    let project_root = crate_path.parent().unwrap().parent().unwrap().to_path_buf();
    ...
    fs_extra::dir::copy(project_root.join("static"), &dist_path, &copyt_options)
